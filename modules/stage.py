...
def inicializar_stage(jugador: dict, pantalla: pg.Surface, nro_stage: int):
    ...
    stage_data['coords_final_mazo_enemigo'] = (190,70)

    stage_data['coords_inicial_mazo_player'] = (20,360)
    stage_data['coords_final_mazo_player'] = (190,360)
...
def setear_ganador(stage_data: dict, participante: dict):
    puntaje_extra = stage_data.get('stage_timer')
    ...
    print(f"Score antes del bonus: {puntaje_actual} -> {puntaje_actual_nuevo}")
...
def chequear_ganador(stage_data: dict):
    jugador = stage_data.get('jugador')
    enemigo = stage_data.get('enemigo')
    hay_tiempo_disponible = stage_data.get('stage_timer') > 0 

    jugador_perdio = (particip_juego.get_hp_participante(jugador) <= 0 or\
        (particip_juego.get_hp_participante(jugador) < particip_juego.get_hp_participante(enemigo)))
            
    enemigo_perdio = (particip_juego.get_hp_participante(enemigo) <= 0 or\
        (particip_juego.get_hp_participante(enemigo) < particip_juego.get_hp_participante(jugador)))
        
    enemigo_sin_cartas = len(particip_juego.get_cartas_restantes_participante(enemigo)) == 0

    jugador_sin_cartas = len(particip_juego.get_cartas_restantes_participante(jugador)) == 0
